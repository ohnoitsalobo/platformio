...
SimplePatternList gPatterns = { audio_rainbow };
...
uint8_t matrix_L[NUM_LEDS][BANDS+1];
uint8_t matrix_R[NUM_LEDS][BANDS+1];
...
void audio_rainbow(){ // interpolated rainbow
    for(int i = 0; i < NUM_LEDS/2; i++){
        matrix_L[i][BANDS] = 0;                      // set sum of all bands = 0
        matrix_R[i][BANDS] = 0;                      // set sum of all bands = 0
        for(int j = 0; j < BANDS; j++){
            float val_L  = Lpeak[j] / MAX; // val =  val*2 - val*val/255;
            float val_R  = Rpeak[j] / MAX; // val =  val*2 - val*val/255;
            int dist = 4;
            int offset = abs (j * dist - i+2);  // offset the centers of the frequency bands
            if ( offset > dist ) {                 // if outside bounds
                matrix_L[i][j] = CRGB::Black;        // black for that frequency
                matrix_R[i][j] = CRGB::Black;
            } else {                                 // color intensity interpolates with distance from frequency center
                matrix_L[i][j] = (1 - ((float)offset/(float)dist)) * val_L*255;
                matrix_R[i][j] = (1 - ((float)offset/(float)dist)) * val_R*255;
            }
            matrix_L[i][BANDS] += matrix_L[i][j];      // calculate sum
            matrix_R[i][BANDS] += matrix_R[i][j];      // calculate sum
            yield();
        }
        LEFT[NUM_LEDS/2-1-i]  = CHSV((224*i)/(NUM_LEDS/2.0), map(matrix_L[i][BANDS], 0, 255, 255, 200), matrix_L[i][BANDS]); // set hue and brightness
        RIGHT[i] = CHSV((224*i)/(NUM_LEDS/2.0), map(matrix_R[i][BANDS], 0, 255, 255, 200), matrix_R[i][BANDS]); // set hue and brightness
