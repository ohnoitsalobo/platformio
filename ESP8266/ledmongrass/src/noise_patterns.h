...
void noise1() {
    ...
    for (uint16_t i = 0; i < NUM_LEDS; i++) {
        ...
        uint32_t real_x = (coordsX[i] + shift_x) * scale;       // calculate the coordinates within the noise field
        uint32_t real_y = (coordsY[i] + shift_y) * scale;       // based on the precalculated positions

        uint8_t noise = inoise16(real_x, real_y, 4223) >> 8;           // get the noise data and scale it down

        uint8_t index = noise + gHue1;                        // map led color based on noise data
        uint8_t bri   = noise;
...
void noise3() {
    ...
    for (uint16_t i = 0; i < NUM_LEDS; i++) {
        ...
        uint8_t index = noise*2 + gHue1;                        // map led color based on noise data
        uint8_t bri   = noise;
