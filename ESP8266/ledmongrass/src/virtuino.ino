...
void onReceived(char variableType, uint8_t variableIndex, String valueAsText){
    if (variableType=='V'){
        ...
        if (variableIndex < 3){
            ...
            bool all_off = (!V[12] && !V[13] && !V[14] && !V[15]);
            if(V[12] || all_off ) manualColor_LEFT  = manualColor;
            if(V[13] || all_off ) manualColor_RIGHT = manualColor;
            if(V[14] || all_off ) manualColor_UP    = manualColor;
            if(V[15] || all_off ) manualColor_DOWN  = manualColor;
        }
